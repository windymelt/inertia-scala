...
### 3. フラグメント付きリダイレクト（`X-Inertia-Redirect`） ✅（ただしクライアント未対応）
...
注意: サーバー側は inertia-laravel の `Header.php`（`REDIRECT = 'X-Inertia-Redirect'`）準拠で
実装したが、現行クライアント `@inertiajs/core` 2.3.18 は `x-inertia-redirect` を処理しない
（409 の特別扱いは `x-inertia-location` 付きのみ）。このためサンプルでは使用せず、通常の
303 リダイレクトを用いている。クライアントが対応するまで実用上は機能しない。
